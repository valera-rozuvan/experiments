...
//     Wed Apr  2 03:43:14 EEST 2014
//
// Description:
// ------------
//     This JSFiddle tries to get two functions
// simultaneously executing. At first, both functions are
// fairly equal in terms of what they are doing (CPU cycles).
// However, if the variable `enableHeavyFunction` is assigned
// to `true`, then the first function will become very CPU
// cycle intensive.
//
// The output of both functions can be seen. Side by side comparison
// lets for easier observation of both function's progress.
//
// Try changing the value of `enableHeavyFunction`, and running this
// JSFiddle a couple of times.
//
// Main article:
// -------------
//     http://valera-rozuvan.github.io/nintoku/new/age/javascript/concurrency/the-new-age-of-javascript-concurrency/

(function ($) {
    var interval1, interval2,
        ticks1 = 0, ticks2 = 0,
        firstTick1, firstTick2,
        maxTicks = 20,
        intervalTime = 20, // In milliseconds.
        enableHeavyFunction = false;
    ...
    function onReady() {
        $('#out1').append('ready!<br /><br />');
        $('#out2').append('ready!<br /><br />');

        interval1 = window.setInterval(outUpdate1, intervalTime);
        interval2 = window.setInterval(outUpdate2, intervalTime);
    }
    ...
    function outUpdate1() {
        ...
        if (enableHeavyFunction) {
            // heavyFunction1();
            heavyFunction2();
        }

        if (!firstTick1) {
            firstTick1 = currentTick;
        }

        timePassed = currentTick - firstTick1;

        if (ticks1 > 0 && ticks1 <= maxTicks) {
            $('#out1').append('tick ' + ticks1 + ': time = ' + timePassed + ' <br />');
        } else if (ticks1 > maxTicks) {
    ...
    function outUpdate2() {
        ...
        if (!firstTick2) {
            firstTick2 = currentTick;
        }

        timePassed = currentTick - firstTick2;

        if (ticks2 > 0 && ticks2 <= maxTicks) {
            $('#out2').append('tick ' + ticks2 + ': time = ' + timePassed + ' <br />');
        } else if (ticks2 > maxTicks) {
    ...
    function heavyFunction1() {
        ...
        fft = new FFT(
            Math.floor(Math.random() * 1000000) * 0.5,
            Math.floor(Math.random() * 1000000)
        );

        // Cleaning up.
        window.DSP = undefined;
        window.FFT = undefined;
    }
...
function defineDSP() {
// These will be redefined at the end of this function.
// This is what the DSP.js exports.
window.DSP = undefined;
window.FFT = undefined;
...
// Hack to get IE working with this JS Fiddle.
// http://www.remvst.com/article-15-setters-getters-for-internet-explorer-9.html?PHPSESSID=a4r79r4c04065fh6amcftvvir4
function patchIE() {
try {
